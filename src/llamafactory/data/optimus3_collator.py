...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    r"""Data collator that supports Optimus3.
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        for feature in features:
            ...
            # ** optimus3 **
            batch_tasks.append(feature.pop("tasks", None))
        ...
        custom_input = {"tasks": torch.tensor(batch_tasks).reshape(-1, 1)}
        features.update(custom_input)

        if self.model is not None and hasattr(self.model, "get_rope_ipdex"):  # for qwen2vl mrope
            rope_index_kwargs = {
...
@dataclass
class Optimus3SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
